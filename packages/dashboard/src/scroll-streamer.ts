...
export interface ScrollStreamer {
  add(deltaY: number, now: number, nx?: number, ny?: number): void;
...
export function createScrollStreamer(options: {
  ...
  let lastNx: number | undefined;
  let lastNy: number | undefined;
  ...
  const send = (): void => {
    ...
    if (deltaY !== 0) {
      options.sendCommand({ type: 'scroll', deltaY, x: lastNx, y: lastNy });
    }
  };

  return {
    add(deltaY, now, nx, ny) {
      accumulated += deltaY;
      if (nx !== undefined) lastNx = nx;
      if (ny !== undefined) lastNy = ny;
