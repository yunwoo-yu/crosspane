...
- **브라우저는 한 번 묻고, 인앱 웹뷰는 묻지 않습니다.** 크롬은 로컬 네트워크 접근 권한을
  묻습니다 — 허용하세요. 안드로이드 웹뷰(카카오톡·인스타그램·라인 인앱브라우저가 이것입니다)
  안에서는 크롬이 로컬 네트워크 접근을 **무조건 허용**합니다. 팝업조차 뜨지 않습니다 —
  웹뷰에는 호스트 앱이 새 권한 유형을 부여할 방법이 없기 때문입니다.
- **Android 17에서 바뀝니다.** API 37+ 타겟 앱은 `ACCESS_LOCAL_NETWORK` 런타임 권한이
  필수가 되고, **웹뷰 트래픽은 호스트 앱의 권한 상태를 상속**합니다. 즉 Android 17에서는
  카카오톡(또는 그 앱)이 그 권한을 갖고 있어야 인앱브라우저가 여러분의 노트북에 닿는데,
  그 앱들이 그 권한을 요청할 이유는 거의 없습니다. `--lan-tls`는 **지금 잘 되지만 언젠가
  막힐 수 있는 길**로 보세요. 토대로 삼지 마세요. iOS는 14부터 이미 그렇습니다 —
  로컬 네트워크 검사가 `WKWebView`에도 적용되고 호스트 앱이
  `NSLocalNetworkUsageDescription`을 선언해야 합니다.
- **오프라인 캡처는 아무도 못 막는 길입니다.** `agent.copyCapture()`는 네트워크도,
  인증서도, 권한도 필요 없습니다. 잠긴 빌드의 최후 수단으로 남는 이유입니다.
...
- **네트워크** — 가로챌 수 있는 것만이 아니라 **페이지가 낸 모든 요청**. fetch와 XHR은
  훅으로(상태·소요시간·실패, 본문 미리보기는 선택), 이미지·CSS·script·`sendBeacon`·
  `EventSource`와 **에이전트가 로드되기 전에 나간 요청**은 리소스 타이밍으로 복구합니다.
  필터에 가려진 건수를 화면에 항상 표시하므로, 빈 목록이 "기록 못 함"을 뜻하는 일이 없습니다
- **타임라인** — 로그·요청·클릭·성능을 **한 줄기**로, 종류별로 걸러 봅니다. 개발자도구는
  이것들이 탭으로 갈라져 있어 인과를 머릿속에서 다시 맞춰야 하는데, 여기서는
  `click button#pay` → `POST 500 /api/pay` → `결제 실패`가 세 줄로 이어집니다
- **사용자 상호작용** — 클릭·폼 제출·조작 키, 그리고 입력 **길이**. 무엇을 쳤는지는
  담지 않으므로 비밀번호가 로그로 샐 수 없습니다
- **렌더링·응답성** — LCP·CLS·INP·FCP·TTFB와 long task. 개발자도구가 없는 웹뷰에서
  "왜 느리지"에 답할 수 있습니다
